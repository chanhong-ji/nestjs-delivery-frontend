...
export default function ShoppingBag({
    ...
    return (
        <form
            className='p-7 bg-slate-50 border cursor-pointer transition-all h-min xl:col-span-1 col-span-3 mt-5'
            onSubmit={onSubmit}
        >
            <h4 className='text-xl font-semibold'>장바구니</h4>
            {fields.map((order, index) => {
                const dish = menu.find((value) => value.id === order.dishId);
                if (!dish) return null;
                ...
                return (
                    ...
                    >
                        <div className='flex justify-between mb-1'>
                            <h3 id='title' className='font-semibold text-sm'>
                                {dish.name}
                            </h3>
                            <button
